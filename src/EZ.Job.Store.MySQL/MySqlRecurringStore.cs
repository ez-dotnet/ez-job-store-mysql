...
namespace EZJob.Store.MySQL;

public sealed class MySqlRecurringStore : IRecurringStore
{
    ...
    public MySqlRecurringStore(string connectionString)
    {
    ...
    public async ValueTask RemoveAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        await using var cmd = new MySqlCommand("DELETE FROM ez_recurring_definitions WHERE id = @id", conn);
    ...
    public async ValueTask<RecurringDefinition?> GetAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        await using var cmd = new MySqlCommand("SELECT * FROM ez_recurring_definitions WHERE id = @id", conn);
        cmd.Parameters.AddWithValue("@id", id.ToString());
    ...
    public async ValueTask<IEnumerable<RecurringDefinition>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using var cmd = new MySqlCommand("SELECT * FROM ez_recurring_definitions ORDER BY created_at_utc", conn);
    ...
    public async ValueTask SetActiveAsync(Guid id, bool isActive, CancellationToken cancellationToken = default)
    {
        ...
        await using var cmd = new MySqlCommand("UPDATE ez_recurring_definitions SET is_active = @is_active WHERE id = @id", conn);
    ...
    private static void AddParameters(MySqlCommand cmd, RecurringDefinition def)
    {
    ...
    private static RecurringDefinition ReadDefinition(MySqlDataReader reader)
    {
